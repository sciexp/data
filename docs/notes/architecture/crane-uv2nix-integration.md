...
This document describes the target architecture for integrating Rust extension modules into omicsio's federated Python monorepo pattern.
...
The `mkEditablePythonSet` in `nix/modules/python.nix` composes editable overlays only for pure Python packages (`pnt-functional`, `omicsio`), excluding `pnt-cli`.
